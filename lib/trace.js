...
(function () {
  ...
  Trace = function(name, options) {
    ...
    ['traceId', 'spanId', 'parentSpanId', 'debug'].forEach(function(idName) {
  ...
  Trace.prototype.toHeaders = function (headers) {
    ...
    if (self.parentSpanId !== undefined && self.parentSpanId) {
   ...
   * @param {Object} hostAddress host address object containing 'address' and 'port'
   */
  Trace.fromRequest = function(request, serviceName, hostAddress) {
    var newTrace = Trace.fromHeaders(
      request.method || 'GET', request.headers || {});

    var host;
    if (hostAddress === undefined || hostAddress === null) {
      host = (request.socket && request.socket.address ?
